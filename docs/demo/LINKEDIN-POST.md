...
Built on **Next.js 16 + TypeScript**, with **26 automated tests** proving the security properties hold (the audit chain detects tampering, dual-control can’t be bypassed), and **0 npm audit vulnerabilities** along with a full threat model in `SECURITY.md`.
...
- **Upload the architecture diagram** (`chokepoint/public/architecture.svg` → save as PNG) or a screenshot of the dashboard as the image — visuals get far more engagement.
